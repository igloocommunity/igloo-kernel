...
static int tc35892_gpio_irq_set_type(struct irq_data *data, unsigned int type)
{
	struct tc35892_gpio *tc35892_gpio = irq_data_get_irq_chip_data(data);
	int offset = data->irq - tc35892_gpio->irq_base;
...
static void tc35892_gpio_irq_lock(struct irq_data *data)
{
	struct tc35892_gpio *tc35892_gpio = irq_data_get_irq_chip_data(data);
...
static void tc35892_gpio_irq_sync_unlock(struct irq_data *data)
{
	struct tc35892_gpio *tc35892_gpio = irq_data_get_irq_chip_data(data);
	struct tc35892 *tc35892 = tc35892_gpio->tc35892;
...
static void tc35892_gpio_irq_mask(struct irq_data *data)
{
	struct tc35892_gpio *tc35892_gpio = irq_data_get_irq_chip_data(data);
	int offset = data->irq - tc35892_gpio->irq_base;
	int regoffset = offset / 8;
...
static void tc35892_gpio_irq_unmask(struct irq_data *data)
{
	struct tc35892_gpio *tc35892_gpio = irq_data_get_irq_chip_data(data);
	int offset = data->irq - tc35892_gpio->irq_base;
	int regoffset = offset / 8;
...
static int tc35892_gpio_irq_init(struct tc35892_gpio *tc35892_gpio)
{
	...
	for (irq = base; irq < base + tc35892_gpio->chip.ngpio; irq++) {
		irq_set_chip_data(irq, tc35892_gpio);
		irq_set_chip_and_handler(irq, &tc35892_gpio_irq_chip,
					 handle_simple_irq);
